...
export type Item = ItemObject | ItemObject[];

type AllInputs = HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement;
...
function changeSelectColor(event: Event) {
  changeCss({ color: "inherit" }, event.target as HTMLSelectElement);
}
...
export class LabeledControls extends Shadow {
  ...
  updated() {
    ...
    (this.dom.class["control"] as AllInputs[]).forEach((control, i) => {
  ...
  handleFileSelection(event: Event) {
    ...
    if (typeof readingMethod === "string") {
      ...
      (reader as any)[readingMethod](file) as unknown as string | null;
    } else {
      reader.readAsDataURL(file) as unknown as string | null;
    }
    reader.addEventListener("load", (event) => {
      this.inputFile = reader.result as unknown as string | null;
    });
  }

  getInput() {
    return Object.fromEntries(
      (this.dom.class["control"] as AllInputs[]).map((el: AllInputs) => [
        el.id,
  ...
  getFormData() {
    ...
    (this.dom.class["control"] as AllInputs[]).forEach((c) => {
  ...
  reportValidity() {
    return (this.dom.class["control"] as AllInputs[]).every((el) =>
      el.reportValidity()
    );
  }

  reset() {
    const inputsAndTextareas = this.dom.class["control"] as AllInputs[];
    const selects = [
